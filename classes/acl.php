 ...
 * @license    http://www.poomalairaj.com/license/new-bsd     New BSD License
 */
class Acl
{
    /**
    ...
    /**
     * @var Acl_Role_Interface
     */
    protected $_isAllowedRole     = null;

    /**
     * @var Acl_Resource_Interface
     */
     ...
     * @param  Acl_Role_Interface|string|array $parents
     * @uses   Acl_Role_Registry::add()
     * @return Acl Provides a fluent interface
     */
    public function addRole($role, $parents = null)
    {
        if (is_string($role)) {
            $role = new Acl_Role($role);
        }

        if (!$role instanceof Acl_Role_Interface) {
            throw new Acl_Exception('addRole() expects $role to be of type Acl_Role_Interface');
        }
     ...
     *
     * @param  Acl_Role_Interface|string $role
     * @uses   Acl_Role_Registry::get()
     * @return Acl_Role_Interface
     */
     ...
     *
     * @param  Acl_Role_Interface|string $role
     * @param  Acl_Role_Interface|string $inherit
     * @param  boolean                        $onlyParents
     * @uses   Acl_Role_Registry::inherits()
     * @return boolean
     */
     ...
     *
     * @uses   Acl_Role_Registry::removeAll()
     * @return Acl Provides a fluent interface
     */
     ...
     *
     * @param  Acl_Resource_Interface|string $resource
     * @param  Acl_Resource_Interface|string $parent
    ...
    public function addResource($resource, $parent = null)
    {
        if (is_string($resource)) {
            $resource = new Acl_Resource($resource);
        }

        if (!$resource instanceof Acl_Resource_Interface) {
            throw new Acl_Exception('addResource() expects $resource to be of type Acl_Resource_Interface');
        }

        $resourceId = $resource->getResourceId();

        if ($this->has($resourceId)) {
            throw new Acl_Exception("Resource id '$resourceId' already exists in the ACL");
        }
        ...
        if (null !== $parent) {
            try {
                if ($parent instanceof Acl_Resource_Interface) {
                ...
                throw new Acl_Exception("Parent Resource id '$resourceParentId' does not exist", 0, $e);
            }
     ...
     * @return Acl Provides a fluent interface
     */
    public function add(Acl_Resource_Interface $resource, $parent = null)
    {
    ...
    public function get($resource)
    {
        ...
        if (!$this->has($resource)) {
            throw new Acl_Exception("Resource '$resourceId' not found");
        }
     ...
     *
     * @param  Acl_Resource_Interface|string $resource
     * @param  Acl_Resource_Interface|string $inherit
     * @param  boolean                            $onlyParent
     * @throws Acl_Resource_Registry_Exception
     * @return boolean
     */
    public function inherits($resource, $inherit, $onlyParent = false)
    {
        try {
            ...
            throw new Acl_Exception($e->getMessage(), $e->getCode(), $e);
        }
     ...
     *
     * @param  Acl_Role_Interface|string|array     $roles
     * @param  Acl_Resource_Interface|string|array $resources
     * @param  string|array                             $privileges
     * @param  Acl_Assert_Interface                $assert
     * @uses   Acl::setRule()
     * @return Acl Provides a fluent interface
     */
    public function allow($roles = null, $resources = null, $privileges = null, Acl_Assert_Interface $assert = null)
    {
     ...
     * @return Acl Provides a fluent interface
     */
    public function deny($roles = null, $resources = null, $privileges = null, Acl_Assert_Interface $assert = null)
    {
    ...
    public function setRule($operation, $type, $roles = null, $resources = null, $privileges = null,
                            Acl_Assert_Interface $assert = null)
    {
        // ensure that the rule type is valid; normalize input to uppercase
        $type = strtoupper($type);
        if (self::TYPE_ALLOW !== $type && self::TYPE_DENY !== $type) {
            throw new Acl_Exception("Unsupported rule type; must be either '" . self::TYPE_ALLOW . "' or '"
                                       . self::TYPE_DENY . "'");
        }
        ...
        switch ($operation) {
            ...
            default:
                throw new Acl_Exception("Unsupported operation; must be either '" . self::OP_ADD . "' or '"
                                           . self::OP_REMOVE . "'");
        }
    ...
    public function isAllowed($role = null, $resource = null, $privilege = null)
    {
        ...
        if (null !== $role) {
            ...
            if (!$this->_isAllowedRole instanceof Acl_Role_Interface) {
                $this->_isAllowedRole = $role;
            }
        }
        ...
        if (null !== $resource) {
            ...
            if (!$this->_isAllowedResource instanceof Acl_Resource_Interface) {
                $this->_isAllowedResource = $resource;
            }
        }
     ...
     *
     * @return Acl_Role_Registry
     */
    protected function _getRoleRegistry()
    {
        if (null === $this->_roleRegistry) {
            $this->_roleRegistry = new Acl_Role_Registry();
        }
     ...
     * @return boolean|null
     */
    protected function _roleDFSAllPrivileges(Acl_Role_Interface $role, Acl_Resource_Interface $resource = null)
    {
        $dfs = array(
            'visited' => array(),
            'stack'   => array()
     ...
     * @throws Acl_Exception
     */
    protected function _roleDFSVisitAllPrivileges(Acl_Role_Interface $role, Acl_Resource_Interface $resource = null,
                                                 &$dfs = null)
    {
        if (null === $dfs) {
            throw new Acl_Exception('$dfs parameter may not be null');
        }
     ...
     * @throws Acl_Exception
     */
    protected function _roleDFSOnePrivilege(Acl_Role_Interface $role, Acl_Resource_Interface $resource = null,
                                            $privilege = null)
    {
        if (null === $privilege) {
            throw new Acl_Exception('$privilege parameter may not be null');
        }
     ...
     * @throws Acl_Exception
     */
    protected function _roleDFSVisitOnePrivilege(Acl_Role_Interface $role, Acl_Resource_Interface $resource = null,
     ...
     * @return string|null
     */
    protected function _getRuleType(Acl_Resource_Interface $resource = null, Acl_Role_Interface $role = null,
                                    $privilege = null)
    {
        ...
        if ($rule['assert']) {
            ...
            $assertionValue = $assertion->assert(
                $this,
                ($this->_isAllowedRole instanceof Acl_Role_Interface) ? $this->_isAllowedRole : $role,
                ($this->_isAllowedResource instanceof Acl_Resource_Interface) ? $this->_isAllowedResource : $resource,
     ...
     * @return array|null
     */
    protected function &_getRules(Acl_Resource_Interface $resource = null, Acl_Role_Interface $role = null,
                                  $create = false)
    {
     ...
     */
     ...
     {
	$serialized_acl = serialize($this);
	file_put_contents('/tmp/acl.data', $serialized_acl);
     }
     
     public function loadAcl($type = 'file') 
     {
     	return unserialize(file_get_contents('/tmp/acl.data'));
